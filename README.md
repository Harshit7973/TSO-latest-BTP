...
The main class is [SumoEnvironment](sumo_rl/environment/env.py).
...
For multiagent environments, use [env](sumo_rl/environment/env.py) or [parallel_env](sumo_rl/environment/env.py) to instantiate a [PettingZoo](https://github.com/PettingZoo-Team/PettingZoo) environment with AEC or Parallel API, respectively.
[TrafficSignal](sumo_rl/environment/traffic_signal.py) is responsible for retrieving information and actuating on traffic lights using [TraCI](https://sumo.dlr.de/wiki/TraCI) API.
...
You can define your own observation by implementing a class that inherits from [ObservationFunction](sumo_rl/environment/observations.py) and passing it to the environment constructor.
...
E.g.: In the [2-way single intersection](experiments/dqn_2way-single-intersection.py) there are |A| = 4 discrete actions, corresponding to the following green phase configurations:
...
You can choose a different reward function (see the ones implemented in [TrafficSignal](sumo_rl/environment/traffic_signal.py)) with the parameter `reward_fn` in the [SumoEnvironment](sumo_rl/environment/env.py) constructor.
...
Check [experiments](experiments) for examples on how to instantiate an environment and train your RL agent.

### [Q-learning](agents/ql_agent.py) in a one-way single intersection:
```bash
